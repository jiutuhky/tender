// 角色引擎 · 眼睛层 —— grok-icon-study replica src/eyes.js 的 1:1 移植。
//
// 眨眼队列（四关键帧 + 14% 二连眨）、单眼 wink、以及 paintEyes：两只多边形眼睛在
// 面上的定位。眼睛的 3D 定位走 relRot 旋转差的切标架（转到侧面自己压扁淡出），
// 转头时沿轮廓环的 liveSpan 缠绕；通知徽章把眼睛推开。数值全部对齐原版。

import {
  centroid, clamp, Dke, polyPath, spanAt, spanPoly, type FaceVal, type Poly,
} from "./math";
import type { ShapeGeo } from "./geometry";

export interface BlinkKey {
  at: number;
  v: number;
}

export function queueBlink(q: BlinkKey[], now: number): void {
  q.push(
    { at: now, v: 0.05 },
    { at: now + 70, v: 0.05 },
    { at: now + 150, v: 1.08 },
...
export function consumeBlink(q: BlinkKey[], now: number): number | null {
  ...
  while (q.length && now >= (q[0]?.at ?? Infinity)) key = q.shift()?.v ?? null;
  return key;
}

/** 单眼眨：320ms 一个来回，闭得比双眼眨慢一点，才看得出是「故意的」 */
export function winkLid(base: number, now: number, winkAt: number, winkEye: number, i: number): number {
...
export interface PaintEyesOpt {
  now: number;
  /** 两只眼的当前多边形（已按 morphT 插值） */
  polys: [Poly, Poly];
  morphT: number;
  shape: ShapeGeo;
  face: FaceVal;
  faceTune: { size: number; gap: number; height: number; eyeWidth: number; eyeHeight: number } | null;
  uniformEyes: boolean;
  eyeScaleProp: number;
  blinkX: number;
  eyeBoostX: number;
  ...
  winkAt: number;
  winkEye: number;
  /** 偏航角（弧度）：非 null 时眼睛沿轮廓缠绕 */
  turn: number | null;
  /** relRot 旋转差：非 null 时走 3D 定位 */
  cr: Mat3Like | null;
  pointer: { x: number; y: number } | null;
  notifyX: number;
  overlayX: number;
  eyeEls: SVGPathElement[];
  badgeEl: SVGCircleElement | null;
  badgeColor: string;
  Re: number;
  G9e: number;
  VJt: number;
  extras: { Zr?: number; wi?: number };
  ringHint: [number, number][] | null;
  badgeRing: readonly [number, number][] | null;
  top: number;
  bottom: number;
  emphasisBlend: number;
}

type Mat3Like = number[];

export function paintEyes(opt: PaintEyesOpt): void {
  const {
  ...
  const sX = uniformEyes ? ($i.leftDX ?? 0) : 0;
  const cents = [centroid(polys[0] as Poly), centroid(polys[1] as Poly)];
  let a1 = 0;
  let o1 = 0;
  for (const p of polys[0] as Poly) a1 = Math.max(a1, Math.abs((p[0] as number) - cents[0]![0]));
  for (const p of polys[1] as Poly) o1 = Math.max(o1, Math.abs((p[0] as number) - cents[1]![0]));
  ...
  const liveSpan = ringHint
    ? (y: number) => spanPoly(ringHint, y, Re)
  ...
  const Yl = badgeRing
    ? badgeRing[Math.round(badgeRing.length * 7 / 8) % badgeRing.length] as [number, number]
    : [Re, shape.top] as [number, number];

  for (let i = 0; i < 2; i++) {
    const poly = polys[i] as Poly;
    const [Gn, Ti] = cents[i] as [number, number];
    const eyeEl = eyeEls[i] as SVGPathElement;
    eyeEl.setAttribute("d", polyPath(poly));
    ...
    eyeEl.style.display = bre && overlayX < 0.5 ? "" : "none";
    ...
    for (let p = 0; p < poly.length; p += 2) {
      const Frp = (((poly[p] as number[])[0] as number) - Gn) * Vee;
      const [Ia2, li2] = liveSpan(vl + (((poly[p] as number[])[1] as number) - Ti) * _2);
