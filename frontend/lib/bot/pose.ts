// 角色引擎 · 姿态层 —— grok-icon-study replica src/pose.js 的 1:1 移植。
//
// applyPose 按 39 态 switch 给出 spin/tx/ty/squash/lid/eyeBoost，动作里的一次性事件
// （点头、甩头、转圈请求、眨眼请求）通过 ctx 递给主循环；nextGaze 给出各状态的
// 视线落点与停留窗口。数值全部对齐原版。

import { clamp, K2, rand, sign } from "./math";
import { EYE_PLAYLIST, type BotState } from "./tables";

/** 姿态层可变上下文：节拍计时器与一次性请求，每个实例一份（原 ctx / _freshCtx 的姿态半边） */
export interface PoseCtx {
  nodUntil: number;
  nodEnd: number;
  angryShakeUntil: number;
  impulseAt: number;
  tyKick: number;
  spinKick: number;
  forceSleepEye: boolean;
  wakeEye: [number, number] | null;
  wakeBlink: boolean;
  wakingBlinked: boolean;
  slumpAt: number;
  stAt: number;
  wantPn: [number, number] | null;
  wantBlink: boolean;
  dragCycle: number;
  notifyPop: boolean;
  wantBurst: [number, number] | null;
  wakingBurst: boolean;
}

export function freshPoseCtx(now: number): PoseCtx {
  return {
    nodUntil: now + 1800,
    nodEnd: 0,
    angryShakeUntil: 0,
...
export function applyPose(
  ...
  switch (state) {
    case "sleeping": {
      ...
      if ((EYE_PLAYLIST.sleeping as readonly number[]).includes(extra.eyeTo)) {
...
/** 各状态的下一个视线落点与停留窗口（秒） */
export function nextGaze(state: BotState): { x: number; y: number; hold: readonly [number, number] } {
  switch (state) {
    case "idle":
