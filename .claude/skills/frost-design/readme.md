...
- **Color.** Cold-gray neutral ground (`--canvas #EEF0F3` light, `#161618` dark) with **a single accent — system blue** (`#0064E1` light, `#409CFF` dark). Blue means exactly two things: *interactive* and *in-progress*. Decoration never spends color — and neither does light: rim, illumination and specular are white. *Character exception:* when the agent delegates, each subagent's Bot avatar takes an **identity hue** hashed from its call id (golden-angle spread, stable across re-renders) so parallel workers are told apart at a glance — the hue rotates the avatar's own three-stop gradient and stops at the superellipse edge; the row surface, separators, labels and status dots around it stay neutral and system blue. Identity is information, not decoration. Semantic colors ship as **graphic/text pairs** — a saturated value for fills & status dots, a darkened value for legible text on tint (e.g. success `#34C759` graphic / `#1D8F45` text).
...
- **Depth replaces strokes.** Solid surfaces use three shadow steps (`--elev-1/2/3`), each pairing a soft drop shadow with a **.5px ambient ring** (the contact edge). Glass surfaces use five-part stacks (`--glass-shadow-thin/regular/thick`: ambient, mid, near, all-round haze, contact ring) because lens glass has no base colour — the shadow *is* its edge. `.frost-glass` also zeroes `border`: a `<button>` that doesn't set one gets the UA's `2px outset ButtonBorder`, which paints a solid white frame over the whole optical stack. Hairline `--separator` dividers exist for lists; chrome meets content on a `.5px` contact edge (`box-shadow: inset … var(--separator)`), never a 1px border, and bars over scrolling content use `.frost-scroll-edge` (progressive blur) instead of a line.
...
- **Animation.** Answers three questions only — what happened, where from, where to. Durations: micro 120ms / float 200ms / panel 320ms. Standard easing `cubic-bezier(.32,.72,0,1)`. Only `transform` & `opacity` animate (plus the registered `--_spot-a` number on glass); entrance offset ≤ 8px; **no infinite loops** — with exactly two sanctioned families, both bound to a running state and both stopping the moment it ends: (a) the running-state border beam, and (b) the **Prose Bot character's vital signs**. *(b) is a character, not a widget:* the bot is the app icon made animate, and while it works it breathes — a white halo pulsing outward (2.4s), the eyes making a saccade left, hold, right, hold, then a blink on the way back (4.2s), and the body drifting 1.2px with a half-degree head tilt (3.1s); subagent avatars squash-and-stretch on the same terms, phase-offset per row so a board of them never beats in unison. Periods are deliberately non-divisible so the layers never lock into a metronome, amplitudes stay ≤ 1.5px / ≤ 5% scale, only transform & opacity move, and nothing loops outside the avatar's own superellipse. Idle bots hold still. Everything degrades to an instant cut under `prefers-reduced-motion`, which also disables the pointer specular.
...
| ④ | rim | `--glass-rim-w`, `--glass-rim-base`, `--glass-rim-catch`, `--glass-rim-echo` | three inset shadows: a 1px ring all round, plus a 135° pair — top-left catches the light, bottom-right is the internal reflection, flanks keep a thread. Analytic strokes (Skia antialiases a rounded-rect stroke); a masked 1px ring reads as a jagged arc at the corners |
