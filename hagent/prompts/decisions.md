...
## 2026-09-07 增补：任务执行顺序

**ADD**：在 `# Using your tools` 明确按执行步骤逐个创建任务、收到返回 id 后再创建下一项，并在执行前用 `TaskUpdate.addBlockedBy` 写入真实依赖。恢复已有任务时核对并补齐依赖。

原因：创建编号用于无依赖任务的展示顺序；并行创建可能让后续复核阶段先取得编号。进度接口按完整依赖排列前置与后续步骤，完成状态只影响提示，保持步骤位置稳定。

### tiktoken 计量（cl100k_base 编码）

| 文件 | tokens | 行数 |
|---|---|---|
| prompts/hagent_base.zh.md（任务执行顺序增补后） | 4067 | 111 |
