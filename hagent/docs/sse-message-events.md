...
**列表顺序**：后端按完整任务依赖排列，前置任务在后续任务前；每次从可排任务中按创建编号选择第一项。排序包含已完成的前置任务，因此仅状态变化不会引起步骤换位。无依赖时沿用创建编号顺序；缺失或隐藏的任务不参与展示排序；循环依赖涉及的剩余任务按创建编号保留。排序完成后，`blockedBy` 仅保留未完成的前置任务，用于等待提示。`GET /sessions/{sid}/todos` 使用相同规则，前端保持返回顺序。
