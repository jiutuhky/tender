...
(() => {
  ...
  class Bot {
    constructor(host,size,id,{frozen=false,ink=null}={}){
      ...
      this.back=el('g',{'data-layer':'ribbon-back'});this.front=el('g',{'data-layer':'ribbon-front'});this.group=el('g',{'data-layer':'body'});this.body=el('path',{class:'bot-body'});this.face=el('g');this.eyes=[el('path',{class:'bot-eye'}),el('path',{class:'bot-eye'})];this.face.append(...this.eyes);this.group.append(this.body,this.face);this.svg.append(this.back,this.group,this.front);
    ...
    draw(dt,{still=false,reduce=false,trails=true,repeat=false,instantRibbons=false}={}){
      ...
      this.initial=true;const v=this.pose,angle=v.yaw*Math.PI/180,side=Math.cos(angle),width=.76+.24*Math.abs(side);
      this.group.setAttribute('transform',`translate(${v.x} ${v.y}) translate(50 50) rotate(${v.r}) scale(${v.sx*width} ${v.sy}) translate(-50 -50)`);
      this.body.setAttribute('d',bodyPath(v));
      this.face.setAttribute('transform',`translate(${v.gx+Math.sin(angle)*7} ${v.gy}) translate(50 46) scale(${.75+.25*Math.abs(side)} 1) translate(-50 -46)`);
      this.face.setAttribute('opacity',Math.max(.12,(side+1)/2));this.eyes[0].setAttribute('d',eye(40,46,v.lw,Math.max(2,v.lh),v.ls,v.la));this.eyes[1].setAttribute('d',eye(60,46,v.rw,Math.max(2,v.rh),v.rs,v.ra));
