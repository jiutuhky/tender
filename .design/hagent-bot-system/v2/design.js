...
(() => {
  ...
  let reduce=matchMedia('(prefers-reduced-motion: reduce)').matches,trails=true,paused=false,speed=1,selected='thinking',filter='state';
  ...
  window.hagentDesign={version:2,entries:entries.map(({id,group})=>({id,group})),snapshot:()=>({selected,reduce,trails,paused,speed,scene:!!scene,waitingApproval,time:mainBot.time,raf:!!raf,pose:{...mainBot.pose},ribbon:{...mainBot.ribbon},visible:allBots.filter(b=>b.visible).length})};
